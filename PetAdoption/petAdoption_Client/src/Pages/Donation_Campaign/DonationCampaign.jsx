...
const DonationCampaign = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.data.success) {
      ...
      const petInformation = {name, age, category, location, short_desc, long_desc, adoption, petImage, date, user_email, lastDate, amount};

      axiosSecure.post("/donation_camp", petInformation).then((res) => {
    ...
    <div className="my-4">
      <div className="hero min-h-fit ">
        <div className="card shrink-0 w-full max-w-lg  ">
          <SectionTitles title="Donation Campaign" />
          <form className="card-body" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="md:flex justify-between gap-2">
              <div className="form-control ">
                ...
                {/* <input {...register("age", {required: true})} type="text" placeholder="Age" className="input input-bordered select-warning" required /> */}
                <input {...register('amount',{required:true})} type="range" min={25} max="100" defaultValue="25" className="range range-warning" step="25" />
              ...
              <div className="form-control ">
                <label className="label">
                  <span className="label-text">Last Day of camp</span>
                </label>
                <input {...register("date", {required: true})} type="date" placeholder="Dog Location" className="input input-bordered select-warning" required />
              </div>
            </div>
